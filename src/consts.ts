// Site-wide constants — change them here, they update everywhere.
export const SITE_TITLE = 'Sergey Völker';
// Used as the home page <title> — leads with name + role for search results.
export const HOME_TITLE =
  'Sergey Völker — AI Enablement & Program Manager';
// Kept under ~155 characters so search engines show it without cutting it off.
export const SITE_DESCRIPTION =
  'AI Enablement & Program Manager in Berlin. Program management, organizational change, and AI adoption. Co-author of "AI Implementation for Managers".';
export const LINKEDIN_URL = 'https://www.linkedin.com/in/sergeyvoelker/';
export const GITHUB_URL = 'https://github.com/medi-v';
export const BOOK_URL =
  'https://www.amazon.com/Introduction-Implementation-Managers-Non-Technical-Professionals/dp/B0GWT18PDC';

// Public profiles used for the "sameAs" structured data. This helps Google
// connect this site to the same person elsewhere on the web.
// Add more profile URLs here (GitHub, X, Amazon author page, Xing …) as you like.
export const PROFILE_URLS = [
  LINKEDIN_URL,
  GITHUB_URL,
  'https://medium.com/@sergey.medi',
  'https://www.amazon.com/stores/Sergey-V%C3%B6lker/author/B0H3R49925',
];

// Search-engine ownership verification codes.
// Paste the code from Google Search Console / Bing Webmaster Tools here to verify
// via HTML tag. Leave empty ('') if you verify through DNS instead — then no tag
// is added to the page.
export const GOOGLE_SITE_VERIFICATION = '';
export const BING_SITE_VERIFICATION = '';
